...
func TestCertificateActionApprovalBindsExactExportAndRevokeContent(t *testing.T) {
	...
	exportHash, summary, err := certificateActionBinding("certificate.private_key.export", certificateID, nodeID, 7, "certificate_p12", artifactID, "", []byte("certificate-chain"))
	if err != nil || len(summary) == 0 {
		t.Fatalf("export binding err=%v summary=%s", err, summary)
	}
	tamperedVersion, _, _ := certificateActionBinding("certificate.private_key.export", certificateID, nodeID, 8, "certificate_p12", artifactID, "", []byte("certificate-chain"))
	tamperedArtifact, _, _ := certificateActionBinding("certificate.private_key.export", certificateID, nodeID, 7, "certificate_p12", uuid.Must(uuid.NewV7()), "", []byte("certificate-chain"))
	if bytes.Equal(exportHash, tamperedVersion) || bytes.Equal(exportHash, tamperedArtifact) {
		t.Fatal("export approval hash did not bind version and artifact identity")
	}
