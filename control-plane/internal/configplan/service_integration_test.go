...
func TestConfigPlanCreateReplayStaleAndTypedEnvelopeIntegration(t *testing.T) {
	...
	creator := &recordingOperationCreator{delegate: operations.NewBackend(postgres.WrapPool(pool), 50, commandSigner)}
	service := NewBackend(postgres.WrapPool(pool), creator)
	...
	for _, table := range []string{"commands", "config_plans", "audit_events"} {
		if err := pool.QueryRow(ctx, `SELECT count(*) FROM `+table+` WHERE workspace_id=$1`, workspaceID).Scan(&orphaned); err != nil || orphaned != 0 {
			t.Fatalf("atomic failure left %s rows=%d: %v", table, orphaned, err)
		}
	}
	if len(creator.requests) != 1 || creator.contexts[0] != ctx {
		t.Fatal("Create did not use injected creator once with original context")
	}
	creator.requests, creator.contexts = nil, nil
	creator.err = fmt.Errorf("creator: %w", operations.ErrBacklogExceeded)
	if _, replay, err := service.Create(ctx, request); err != creator.err || !errors.Is(err, operations.ErrBacklogExceeded) || replay {
		t.Fatal("downstream error changed", replay, err)
	}
	creator.err = nil
	...
	wantCreate := operations.CreateRequest{
		NodeID: nodeID, IdempotencyKey: request.IdempotencyKey, ExpectedVersion: 1,
		Kind: operations.ConfigPlan, Candidate: candidate, CandidateHash: candidateHash[:],
		TTL: request.TTL, RequestID: request.RequestID, Traceparent: request.Traceparent,
		ActorID: request.ActorID, Action: "config.plan", Reason: request.Reason,
		PlanCapabilities: []string{"config.auth", "config.limits", "config.network", "config.runtime", "ocserv.config.plan"},
		PlanMetadata:     &operations.ConfigPlanMetadata{TemplateName: "baseline", CandidateRedacted: string(candidate), Warnings: []string{"ocserv_version_unobserved"}},
	}
	creator.assertLast(t, ctx, 3, wantCreate)
	changed := request
	changed.Reason = "different content under the same key"
	if _, _, err := service.Create(ctx, changed); !errors.Is(err, ErrIdempotency) {
		t.Fatal("conflicting replay", err)
	}
	if _, _, err := service.Apply(ctx, ApplyRequest{PlanID: plan.ID, ApprovalID: uuid.New(), IdempotencyKey: "pending", Reason: "pending plan"}); !errors.Is(err, ErrStaleRevision) {
		t.Fatal("pending plan accepted", err)
	}
	if _, err := pool.Exec(ctx, `UPDATE node_capabilities SET approved=false WHERE node_id=$1 AND capability='ocserv.config.plan'`, nodeID); err != nil {
		t.Fatal(err)
	}
	if _, _, err := service.Create(ctx, request); !errors.Is(err, ErrCapability) {
		t.Fatal("missing capability accepted", err)
	}
	if len(creator.requests) != 4 {
		t.Fatal("preflight rejection reached creator")
	}
	if _, err := pool.Exec(ctx, `UPDATE node_capabilities SET approved=true WHERE node_id=$1 AND capability='ocserv.config.plan'`, nodeID); err != nil {
		t.Fatal(err)
	}
	...
	applyRequest := ApplyRequest{PlanID: plan.ID, ApprovalID: approvalID, IdempotencyKey: "i16-apply", ActorID: requesterID.String(), ActorIdentityID: requesterID, ActorSessionID: uuid.Must(uuid.NewV7()), RequestID: "request-i16", Traceparent: request.Traceparent, Reason: "apply approved plan"}
	creator.err = fmt.Errorf("creator: %w", operations.ErrStaleRevision)
	if _, replay, err := service.Apply(ctx, applyRequest); err != creator.err || !errors.Is(err, ErrStaleRevision) || replay {
		t.Fatal("Apply downstream error changed", replay, err)
	}
	creator.err = nil
	t.Run("apply-rollback", func(t *testing.T) {
		if _, err := ownerPool.Exec(ctx, `ALTER TABLE audit_events ADD CONSTRAINT pr04_config_apply_audit CHECK (workspace_id <> '`+workspaceID.String()+`' OR action <> 'config.apply') NOT VALID`); err != nil {
			t.Fatal(err)
		}
		t.Cleanup(func() {
			if _, err := ownerPool.Exec(context.Background(), `ALTER TABLE audit_events DROP CONSTRAINT pr04_config_apply_audit`); err != nil {
				t.Error(err)
			}
		})
		...
		var status string
		if err := pool.QueryRow(ctx, `SELECT status FROM approval_requests WHERE id=$1`, approvalID).Scan(&status); err != nil || status != "approved" {
			t.Fatal("approval consumed by failed transaction", status, err)
		}
		var operationsCount, commandsCount, plansCount, applyCount, outboxCount, auditCount int
		if err := pool.QueryRow(ctx, `SELECT
			(SELECT count(*) FROM operations WHERE workspace_id=$1),
			(SELECT count(*) FROM commands WHERE workspace_id=$1),
			(SELECT count(*) FROM config_plans WHERE workspace_id=$1),
			(SELECT count(*) FROM config_apply_operations WHERE workspace_id=$1),
			(SELECT count(*) FROM outbox_events b JOIN commands c ON c.id=b.command_id WHERE c.workspace_id=$1),
			(SELECT count(*) FROM audit_events WHERE workspace_id=$1 AND action='config.apply')`, workspaceID).Scan(&operationsCount, &commandsCount, &plansCount, &applyCount, &outboxCount, &auditCount); err != nil || operationsCount != 1 || commandsCount != 1 || plansCount != 1 || applyCount != 0 || outboxCount != 1 || auditCount != 0 {
			t.Fatalf("rollback rows = %d/%d/%d/%d/%d/%d: %v", operationsCount, commandsCount, plansCount, applyCount, outboxCount, auditCount, err)
		}
	})
	apply, replayed, err := service.Apply(ctx, applyRequest)
	if err != nil || replayed || apply.State != "queued" {
		t.Fatalf("apply=%+v replayed=%v err=%v", apply, replayed, err)
	}
	creator.assertLast(t, ctx, 7, operations.CreateRequest{
		NodeID: nodeID, IdempotencyKey: applyRequest.IdempotencyKey, ExpectedVersion: 1,
		Kind: operations.ConfigApply, Candidate: candidate, CandidateHash: candidateHash[:], ExpectedCurrentHash: currentHash,
		DesiredRevision: 1, ApplyMetadata: &operations.ConfigApplyMetadata{PlanID: plan.ID}, ApprovalID: approvalID,
		TTL: 15 * time.Minute, RequestID: applyRequest.RequestID, Traceparent: applyRequest.Traceparent,
		ActorID: applyRequest.ActorID, ActorIdentityID: requesterID, ActorSessionID: applyRequest.ActorSessionID,
		Action: "config.apply", Reason: applyRequest.Reason,
	})
