...
func (s certificateStore) Get(ctx context.Context, id uuid.UUID) (certificatestore.Certificate, error) {
	return scanCertificate(s.QueryRow(ctx, `SELECT `+certificateColumns+` FROM certificates WHERE id=?`, UUIDBytes(id)))
}

func (s certificateStore) GetByOperation(ctx context.Context, id uuid.UUID) (certificatestore.Certificate, error) {
	return scanCertificate(s.QueryRow(ctx, `SELECT `+certificateColumns+` FROM certificates WHERE operation_id=?`, UUIDBytes(id)))
}

func (s certificateStore) ListNode(ctx context.Context, id uuid.UUID) ([]certificatestore.Certificate, error) {
	rows, err := s.Query(ctx, `SELECT `+certificateColumns+` FROM certificates WHERE node_id=? ORDER BY created_at DESC,id DESC LIMIT 100`, UUIDBytes(id))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s certificateStore) Resource(ctx context.Context, id uuid.UUID) (workspace, node uuid.UUID, err error) {
	err = s.QueryRow(ctx, `SELECT workspace_id,node_id FROM certificates WHERE id=?`, UUIDBytes(id)).Scan(&workspace, &node)
	return
}
