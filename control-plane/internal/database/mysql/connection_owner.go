...
func (s connectionOwnerStore) Acquire(ctx context.Context, t ownerstore.Term, ttl time.Duration) (v ownerstore.Lease, err error) {
	...
	v.Epoch = epoch + 1
	_, err = s.Exec(ctx, `UPDATE connection_owner_fencing SET owner_instance_id=?,owner_incarnation=?,connection_id=?,owner_epoch=?,lease_until=?,updated_at=? WHERE node_id=?`,
		UUIDBytes(t.Identity.InstanceID), t.Identity.Incarnation, t.ConnectionID[:], v.Epoch, v.Until, at, t.NodeID[:])
	return
}
...
func (s connectionOwnerStore) Read(ctx context.Context, node [16]byte) (v ownerstore.State, err error) {
	var connection []byte
	err = s.QueryRow(ctx, `SELECT owner_instance_id,owner_incarnation,connection_id,owner_epoch,lease_until>TIMESTAMPDIFF(MICROSECOND,'2000-01-01',CURRENT_TIMESTAMP(6)) FROM connection_owner_fencing WHERE node_id=?`, node[:]).
