...
func UpsertIdentity(ctx context.Context, tx database.Tx, proposed uuid.UUID, issuer, subject, email, name string, now time.Time) (uuid.UUID, error) {
	...
	if errors.Is(err, database.ErrNotFound) {
		_, err = tx.Exec(ctx, `INSERT INTO identities(id,issuer,subject,email,display_name,created_at,updated_at) VALUES(?,?,?,NULLIF(?,''),NULLIF(?,''),?,?)`, UUIDBytes(proposed), issuer, subject, email, name, now, now)
		return proposed, err
	}
	if err != nil {
		return uuid.Nil, err
	...
	_, err = tx.Exec(ctx, `UPDATE identities SET email=NULLIF(?,''),display_name=NULLIF(?,''),updated_at=? WHERE id=?`, email, name, now, raw)
	return id, err
}
