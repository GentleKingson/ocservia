...
func (s configurationStore) Capabilities(ctx context.Context, id uuid.UUID) ([]string, error) {
	rows, err := s.Query(ctx, `SELECT capability FROM node_capabilities WHERE node_id=$1 AND approved=true AND (capability IN ('ocserv.config.plan','ocserv.config.complete.plan') OR capability LIKE 'config.%') ORDER BY capability`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s configurationStore) ApplyInput(ctx context.Context, id uuid.UUID, key string) (v configurationstore.ApplyInput, err error) {
	err = s.QueryRow(ctx, `SELECT COALESCE(applied.expected_version,n.version),COALESCE(a.desired_revision-1,state.desired_revision,0),c.envelope
		...
		LEFT JOIN operations o ON o.workspace_id=p.workspace_id AND o.idempotency_key=$2
		...
		WHERE p.id=$1`, id, key).Scan(&v.NodeVersion, &v.DesiredRevision, &v.Envelope)
	return
}
