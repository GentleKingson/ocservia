...
// EnablePreSendBarrier configures the development-harness barrier used to
// stop one exact command after its Claim commits and before transport sees it.
// The exact claimed row is extended only after its command ID matches the arm;
// production configuration rejects this hook before startup.
