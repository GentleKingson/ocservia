...
func TestPreSendBarrierGuard(t *testing.T) {
	for _, test := range []struct {
		...
		{"zero lease without barrier", map[string]string{"OCSERV_DATABASE_URL": "postgres://db/test", "OCSERV_TEST_COMMAND_LEASE": "0s"}, false, 0},
		...
		{"zero lease", map[string]string{"OCSERV_DATABASE_URL": "postgres://db/test", "OCSERV_TEST_PRE_SEND_BARRIER_DIR": "/run/g6-pre-send-barrier", "OCSERV_TEST_COMMAND_LEASE": "0s"}, false, 0},
	} {
		t.Run(test.name, func(t *testing.T) {
