...
32-byte session encryption key, and a 32-byte audit checkpoint key. The browser
...
The two key values are 64 lowercase hexadecimal characters. Production startup
fails when OIDC or the checkpoint key is absent. Development bearer
...
separate event. Audit rows and signed checkpoints are append-only, and the
verification endpoint checks the hash chain. A failed audit insert rolls back
