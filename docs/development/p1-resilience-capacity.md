...
submitters, and bounded 2048-item transport queues. Environment variables can
reduce those values for a smoke run but cannot increase them beyond the I08
envelope. Output includes request and completion p50/p95/p99, telemetry count,
path mix, goroutines, Tokio simulator tasks, RSS, file descriptors, and database
pool activity.
