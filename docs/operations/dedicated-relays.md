...
Set `OCSERV_RELAY_SECRET_DIR` to a launcher-owned mode-`0700` directory, install its TLS files as launcher-owned mode `0444`, and install `relay-access-token` as UID/GID 65532 mode `0400`. Set a digest-pinned `OCSERV_RELAY_IMAGE` and the relay hostname in `deploy/production/relay/relay.toml`, then validate and start each relay independently with `deploy/production/relay/compose.sh config --quiet` and `deploy/production/relay/compose.sh up -d`. Production Controller and Agent processes must use `--relay-mode custom`, both HTTPS URLs, and `--relay-token-file`; public/default relays are not a production fallback.
