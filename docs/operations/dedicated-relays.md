...
Set `OCSERV_RELAY_SECRET_DIR`, a digest-pinned `OCSERV_RELAY_IMAGE`, and the relay hostname in `deploy/production/relay/relay.toml`, then start each relay independently. Production Controller and Agent processes must use `--relay-mode custom`, both HTTPS URLs, and `--relay-token-file`; public/default relays are not a production fallback.
