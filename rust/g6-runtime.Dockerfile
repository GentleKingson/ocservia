# Shared G6 harness release builder for every first-party Rust image. One
# workspace compile produces the transportd, probe, tunnel, Agent, and privd
# binaries; each runtime target below only assembles frozen layers from that
# single builder stage, so a release job can never duplicate the dependency
# build across images. The transportd runtime stages mirror
# rust/transportd.Dockerfile exactly; scripts/test-g6-runtime-adapters.sh
# rejects any drift between the two definitions.
...
COPY --from=g6-rust-builder /src/target/release/ocservia-transportd /usr/local/bin/ocservia-transportd
...
COPY --from=g6-rust-builder /src/target/release/ocservia-g6-probe /usr/local/bin/ocservia-g6-probe
...
COPY --from=g6-rust-builder /src/target/release/ocservia-g6-tunnel /ocservia-g6-tunnel
...
COPY --from=g6-rust-builder /src/target/release/ocservia-agent /usr/local/bin/ocservia-agent
COPY --from=g6-rust-builder /src/target/release/ocservia-privd /usr/local/bin/ocservia-privd
