...
pub struct Builder {
    ...
    relay_inactive_cleanup_time: Duration,
}
...
impl Builder {
    ...
    pub async fn bind(self) -> Result<Endpoint, BindError> {
        ...
        let sock_opts = socket::Options {
            ...
            relay_inactive_cleanup_time: self.relay_inactive_cleanup_time,
        };
    ...
    /// Keeps a connection open to every configured relay.
    ///
    /// By default an endpoint keeps only its preferred home relay connected;
    /// non-home relay connections close after an idle timeout. Enabling this
    /// option preconnects every configured relay and keeps those connections
    /// alive, while still publishing only one preferred home relay in the
    /// endpoint address.
    ///
    /// New relay URLs added through [`Endpoint::insert_relay`] are
    /// preconnected. Relay URLs removed through [`Endpoint::remove_relay`]
    /// are disconnected explicitly.
    pub fn keep_relays_connected(mut self, keep_connected: bool) -> Self {
        self.keep_relays_connected = keep_connected;
        self
    }

    /// Overrides the idle timeout for non-home relay connections.
    ///
    /// This is available only to tests that need to exercise relay lifecycle
    /// behavior without waiting for the production timeout.
    #[cfg(feature = "test-utils")]
    pub fn relay_inactive_cleanup_time(mut self, timeout: Duration) -> Self {
        self.relay_inactive_cleanup_time = timeout;
        self
    }
