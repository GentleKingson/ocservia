...
pub(crate) use relay::assert_receive_progress;
...
impl NetworkChangeSender {
    ...
    /// Reconciles relay transports with their shared relay maps.
    pub(crate) async fn relay_map_change(&self) {
        for relay in &self.relay {
            relay.relay_map_change().await;
        }
    }
