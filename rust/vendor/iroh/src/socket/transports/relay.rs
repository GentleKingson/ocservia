...
impl RelayNetworkChangeSender {
    ...
    /// Reconciles live relay connections after the shared relay map changes.
    pub(super) async fn relay_map_change(&self) {
        if self
            .sender
            .send(RelayActorMessage::RelayMapChange)
            .await
            .is_err()
        {
            warn!("unable to reconcile relay map, relay actor already closed");
        }
    }
