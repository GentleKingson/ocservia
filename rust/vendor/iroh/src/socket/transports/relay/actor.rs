...
//!     - Home relays and configured persistent relays never exit for inactivity.
...
struct ActiveRelayActor {
    ...
    is_home_relay: bool,
    /// Whether this actor stays connected even when it is not the home relay.
    keep_connected: bool,
    /// How long an idle non-home actor stays alive.
...
impl ActiveRelayActor {
    fn new(opts: ActiveRelayActorOptions) -> Self {
        ...
        ActiveRelayActor {
            ...
            is_home_relay: false,
            keep_connected,
            inactive_timeout: Box::pin(time::sleep(inactive_cleanup_time)),
            inactive_cleanup_time,
            stop_token,
            metrics,
            my_relay,
    ...
    fn reset_inactive_timeout(&mut self) {
        self.inactive_timeout
            .as_mut()
            .reset(Instant::now() + self.inactive_cleanup_time);
    }
    ...
    async fn run_dialing(&mut self) -> Option<Result<iroh_relay::client::Client, DialError>> {
        ...
        loop {
            tokio::select! {
                ...
                _ = &mut self.inactive_timeout, if !self.is_home_relay && !self.keep_connected => {
                    debug!(timeout = ?self.inactive_cleanup_time, "Inactive, exiting.");
                    break None;
                }
            }
        }
    ...
    async fn run_connected(
        ...
        let res = loop {
            ...
            tokio::select! {
                ...
                _ = &mut self.inactive_timeout, if !self.is_home_relay && !self.keep_connected => {
                    debug!(timeout = ?self.inactive_cleanup_time, "Inactive, exiting (running).");
                    break Ok(());
                }
            }
        };

        if res.is_ok() && self.keep_connected {
            match time::timeout(RELAY_CLOSE_TIMEOUT, client_sink.close()).await {
                Ok(Ok(())) => {}
                Ok(Err(err)) => debug!("Failed to close client sink gracefully: {err:#}"),
                Err(_) => warn!(?RELAY_CLOSE_TIMEOUT, "Timed out closing relay client sink"),
            }
        } else if res.is_ok()
    ...
    async fn run_sending<T>(
        ...
        let res = loop {
            tokio::select! {
                ...
                _ = &mut self.inactive_timeout, if !self.is_home_relay && !self.keep_connected => {
                    debug!(timeout = ?self.inactive_cleanup_time, "Inactive, exiting (sending).");
                    break Ok(());
                }
            }
        };
...
pub(super) enum RelayActorMessage {
    ...
    /// Reconcile active actors with the shared relay map.
    RelayMapChange,
}
...
pub(crate) struct Config {
    ...
    /// Keep an active connection to every configured relay.
    pub keep_relays_connected: bool,
    /// Idle timeout for non-home relay connections.
    pub relay_inactive_cleanup_time: Duration,
}
...
impl RelayActor {
    ...
    pub(super) async fn run(
        ...
        self.reconcile_relay_map().await;
    ...
    async fn handle_msg(&mut self, msg: RelayActorMessage) {
        match msg {
            ...
            RelayActorMessage::RelayMapChange => self.reconcile_relay_map().await,
        }
    }
    ...
    async fn on_network_change(&mut self, report: Report) {
        ...
        let preferred_relay = if self.config.keep_relays_connected {
            if report
                .preferred_relay
                .as_ref()
                .is_some_and(|url| !self.config.relay_map.contains(url))
            {
                return;
            }
            report.preferred_relay
        } else {
            report.preferred_relay
        };
        if preferred_relay.as_ref() == prev_url {
            // No change.
            return;
        }

        if let Some(relay_url) = preferred_relay {
    ...
    fn active_relay_handle(&mut self, url: RelayUrl) -> ActiveRelayHandle {
        match self.active_relays.get(&url) {
            Some(e) => e.clone(),
            None => {
                let relay_config = self.config.relay_map.get(&url);
                let persistent = self.config.keep_relays_connected && relay_config.is_some();
                self.add_active_relay(url, relay_config, persistent)
            }
        }
    }
    ...
    fn add_active_relay(
        &mut self,
        url: RelayUrl,
        relay_config: Option<Arc<RelayConfig>>,
        persistent: bool,
    ) -> ActiveRelayHandle {
        let handle = self.start_active_relay(url.clone(), relay_config, persistent);
    ...
    fn start_active_relay(
        ...
        let abort_handle = self.active_relay_tasks.spawn(
            async move {
                actor.run().await;
            }
            .instrument(span),
    ...
    async fn reap_active_relays(&mut self) {
        ...
        if let Some(status) = self.config.my_relay.get() {
            if self.config.keep_relays_connected && !self.config.relay_map.contains(status.url()) {
                self.config.my_relay.clear();
            } else {
    ...
    /// Starts newly configured persistent relays and explicitly stops removed relays.
    async fn reconcile_relay_map(&mut self) {
        if !self.config.keep_relays_connected {
            return;
        }

        let configured = self.config.relay_map.urls::<BTreeSet<_>>();
        if self
            .config
            .my_relay
            .get()
            .is_some_and(|status| !configured.contains(status.url()))
        {
            self.config.my_relay.clear();
        }
        ...
        let removed = self
            .active_relays
            .iter()
            .filter(|(url, handle)| handle.persistent && !configured.contains(*url))
            .map(|(url, _handle)| url.clone())
            .collect::<Vec<_>>();
        for url in removed {
            self.stop_active_relay(&url).await;
        }

        for url in configured {
            let Some(desired_config) = self.config.relay_map.get(&url) else {
                continue;
            };
            let restart = self.active_relays.get(&url).is_some_and(|handle| {
                !handle.persistent
                    || handle.relay_config.as_deref() != Some(desired_config.as_ref())
            });
            if restart && !self.stop_active_relay(&url).await {
                continue;
            }
            if !self.active_relays.contains_key(&url) {
                let Some(current_config) = self.config.relay_map.get(&url) else {
                    continue;
                };
                self.add_active_relay(url, Some(current_config), true);
            }
        }
        self.log_active_relay();
    }

    async fn stop_active_relay(&mut self, url: &RelayUrl) -> bool {
        ...
        warn!(%url, "Timed out stopping relay actor gracefully; aborting task");
        handle.abort_handle.abort();
        if time::timeout(RELAY_CLOSE_TIMEOUT, handle.inbox_addr.closed())
            .await
            .is_err()
        {
            error!(%url, "Relay actor inbox remained open after task abort");
            false
        } else {
            true
        }
