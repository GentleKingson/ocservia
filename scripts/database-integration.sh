...
assert_local_bootstrap_schema() {
  ...
  test "$(docker exec "${container}" psql -U ocservia_owner -d "${database}" -Atc "SELECT \"current_schema\", minimum_compatible_controller_schema FROM controller_schema_compatibility WHERE singleton")" = "32|29"
...
for major in "${POSTGRES_MAJORS[@]}"; do
  ...
  PIDS+=("${pid}")
  wait_for_http "http://127.0.0.1:${api_port}/readyz"
  test "$(docker exec "${container}" psql -U ocservia_owner -d ocservia -Atc "SELECT count(*) FROM schema_migrations")" = "32"
  assert_local_bootstrap_schema "${container}" ocservia
  ...
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia -c \
    "INSERT INTO schema_migrations (version, name, checksum) VALUES (33, '000033_future.up.sql', decode(repeat('00', 32), 'hex'))" >/dev/null
  ...
  if ! grep -Fq 'schema compatibility current schema 32 does not match applied schema version 33' "${TMP_ROOT}/pg${major}-unknown-version.log"; then
