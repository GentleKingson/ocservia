...
for major in "${POSTGRES_MAJORS[@]}"; do
  ...
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" \
    <"${ROOT}/control-plane/migrations/000033_local_auth_attempts.down.sql" >/dev/null
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" -c \
    "DELETE FROM schema_migrations WHERE version=33" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" \
    <"${ROOT}/control-plane/migrations/000032_local_bootstrap.down.sql" >/dev/null
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" -c \
    "DELETE FROM schema_migrations WHERE version=32" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" \
    <"${ROOT}/control-plane/migrations/000031_local_credentials.down.sql" >/dev/null
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" -c \
    "DELETE FROM schema_migrations WHERE version=31" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" \
    <"${ROOT}/control-plane/migrations/000030_node_bootstrap_tokens.down.sql" >/dev/null
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" -c \
    "DELETE FROM schema_migrations WHERE version=30" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" \
    <"${ROOT}/control-plane/migrations/000029_controller_schema_compatibility.down.sql" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d "${clean_database}" \
  ...
  assert_local_bootstrap_schema "${container}" "${clean_database}"
  ...
  # Current backend workflows require schema 36. Keep the independent schema-33
  ...
  test "$(docker exec "${container}" psql -U ocservia_owner -d ocservia -Atc "SELECT has_table_privilege('ocservia_app','controller_schema_compatibility','SELECT')")" = "t"
  ...
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia <"${ROOT}/control-plane/migrations/000033_local_auth_attempts.down.sql" >/dev/null
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia -c "DELETE FROM schema_migrations WHERE version=33" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia <"${ROOT}/control-plane/migrations/000032_local_bootstrap.down.sql" >/dev/null
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia -c "DELETE FROM schema_migrations WHERE version=32" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia <"${ROOT}/control-plane/migrations/000031_local_credentials.down.sql" >/dev/null
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia -c "DELETE FROM schema_migrations WHERE version=31" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia <"${ROOT}/control-plane/migrations/000030_node_bootstrap_tokens.down.sql" >/dev/null
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia -c "DELETE FROM schema_migrations WHERE version=30" >/dev/null
  docker exec -i "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia <"${ROOT}/control-plane/migrations/000029_controller_schema_compatibility.down.sql" >/dev/null
  ...
  if ! grep -Fq 'schema compatibility current schema 36 does not match applied schema version 37' "${TMP_ROOT}/pg${major}-unknown-version.log"; then
