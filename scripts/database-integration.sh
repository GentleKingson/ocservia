...
for major in "${POSTGRES_MAJORS[@]}"; do
  ...
  test "$(docker exec "${container}" psql -U ocservia_owner -d ocservia -Atc "SELECT \"current_schema\", minimum_compatible_controller_schema FROM controller_schema_compatibility WHERE singleton")" = "29|29"
  ...
  PIDS+=("${pid}")
  wait_for_http "http://127.0.0.1:${api_port}/readyz"
  test "$(docker exec "${container}" psql -U ocservia_owner -d ocservia -Atc "SELECT count(*) FROM schema_migrations")" = "29"
  test "$(docker exec "${container}" psql -U ocservia_owner -d ocservia -Atc "SELECT \"current_schema\", minimum_compatible_controller_schema FROM controller_schema_compatibility WHERE singleton")" = "29|29"
  ...
  docker exec "${container}" psql -v ON_ERROR_STOP=1 -U ocservia_owner -d ocservia -c \
    "INSERT INTO schema_migrations (version, name, checksum) VALUES (30, '000030_future.up.sql', decode(repeat('00', 32), 'hex'))" >/dev/null
  ...
  if ! grep -Fq 'schema compatibility current schema 29 does not match applied schema version 30' "${TMP_ROOT}/pg${major}-unknown-version.log"; then
