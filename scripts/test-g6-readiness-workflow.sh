...
for token in \
  ...
  'address = address.with_relay_url(relay)'; do
...
grep -q 'g6rd_write_runtime_result' "${FD_A}" || {
  echo "fd-a must publish a bound runtime result and source manifest" >&2
  exit 1
}
grep -q 'g6rd_write_runtime_result' "${FD_B}" || {
  echo "fd-b must publish a bound runtime result and source manifest" >&2
  exit 1
}
grep -q 'node scripts/g6-pipeline.mjs assemble' "${WORKFLOW}" || {
...
grep -q 'mergePeerEffects' "${ROOT}/scripts/g6-pipeline.mjs" || {
  echo "the assembly layer must merge fd-a durable effects into the full command population" >&2
  exit 1
}
