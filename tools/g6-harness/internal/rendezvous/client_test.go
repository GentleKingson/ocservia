...
func TestWaitDownloadTimesOut(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		fmt.Fprint(response, `{"jobs":[{"name":"Resilience / G6 Formal FD-B: Standby, Promotion & Faults","status":"in_progress","conclusion":"","steps":[]}]}`)
	}))
	defer server.Close()
	options := testOptions(t, server.URL, name)
...
func TestWaitDownloadRejectsPeerFailureBeforeDownload(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case strings.Contains(request.URL.Path, "/artifacts"):
			fmt.Fprintf(response, `{"total_count":1,"artifacts":[{"id":99,"name":%q,"expired":false,"digest":%q}]}`, name, "sha256:"+hexDigest(archive))
		default:
			fmt.Fprint(response, `{"jobs":[{"name":"Resilience / G6 Formal FD-B: Standby, Promotion & Faults","status":"in_progress","conclusion":"","steps":[{"name":"enroll","status":"completed","conclusion":"failure"}]}]}`)
		}
	}))
	defer server.Close()
	options := testOptions(t, server.URL, name)
...
func testOptions(t *testing.T, baseURL, name string) Options {
	...
	return Options{
		...
		PeerJob:              "Resilience / G6 Formal FD-B: Standby, Promotion & Faults",
